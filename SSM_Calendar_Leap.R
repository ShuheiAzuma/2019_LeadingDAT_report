...
AIC_model_1_s <- AIC_ammend(result_model_1_s_fluct$kfs, degree = 3 + 1 + 11)
...
AIC_calendar <- AIC_ammend(result_cale$kfs, degree = 3 + 1 + 11 + 1 + 6)

# culculate a effect

effect_cale <- result_cale$kfs$alphahat %>%
  tail(1) %>% 
  round(1) %>%
  as_tibble() %>% 
  gather()
...
alphahatconf <- predict(result_cale$fit$model, interval = "confidence", level = 0.95) %>% as_tibble()
